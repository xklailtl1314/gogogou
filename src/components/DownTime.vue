<template>
  ...
  <div class="downTime-wrapper">
    ...
    <div class="time" v-show="!isShow">
      <!-- <span class="hour">{{myDay}}</span> <span class="dot">:</span> -->
      <span class="hour">{{myHours}}</span> <span class="dot">:</span>
      <span class="minute">{{myMinutes}}</span> <span class="dot">:</span>
...
export default {
  ...
  mounted () {
    // 计算时间差
    let timeLag = (this.endTime - this.sTime) / 1000
    // 判断当前是否时分秒的值是否大于10
    let add = num => {
